#include <stdio.h>
#include <inttypes.h>

uint32_t GOOD_MIDSTATE[] = { 0xcb18a156,0x735fadbc,0xea5b3fc9,0x8ddadef9,0x6c3031aa,0x50cee616,0x826a915d,0x88010577 };
uint32_t GOOD_DATA[] = { 0x514898af,0x51bc10ac,0x1a011337 };

uint32_t PreCalc[6];

const uint32_t SHA256_K[3] = {
	0x428a2f98, 0x71374491, 0xb5c0fbcf
};

#define rotate(x,y) ((x<<y) | (x>>(sizeof(x)*8-y)))
#define rotr(x,y) ((x>>y) | (x<<(sizeof(x)*8-y)))

#define R(a, b, c, d, e, f, g, h, w, k) \
	h = h + (rotate(e, 26) ^ rotate(e, 21) ^ rotate(e, 7)) + (g ^ (e & (f ^ g))) + k + w; \
	d = d + h; \
	h = h + (rotate(a, 30) ^ rotate(a, 19) ^ rotate(a, 10)) + ((a & b) | (c & (a | b)))

void precalc_hash(uint32_t *state, uint32_t *data)
{
	uint32_t A, B, C, D, E, F, G, H;

	A = state[0];
	B = state[1];
	C = state[2];
	D = state[3];
	E = state[4];
	F = state[5];
	G = state[6];
	H = state[7];

	R(A, B, C, D, E, F, G, H, data[0], SHA256_K[0]);
	PreCalc[0] = H;
	PreCalc[3] = D;
	R(H, A, B, C, D, E, F, G, data[1], SHA256_K[1]);
	PreCalc[1] = G;
	PreCalc[4] = C;
	R(G, H, A, B, C, D, E, F, data[2], SHA256_K[2]);
	PreCalc[2] = F;
	PreCalc[5] = B;
}

int main(int argc, char **argv)
{
	precalc_hash(GOOD_MIDSTATE, GOOD_DATA);
	printf("PreCalc - [ %x, %x, %x, %x, %x, %x ]\n", PreCalc[0], PreCalc[1], PreCalc[2], PreCalc[3], PreCalc[4], PreCalc[5]);
}
